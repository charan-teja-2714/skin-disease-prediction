...
\author{\IEEEauthorblockN{Chinna Gopi Simhadri\,*\orcidlink{0000-0002-8311-7495}}
...
\caption{Proposed system architecture. Dermoscopic images pass through a CV validity filter, preprocessing, U-Net segmentation, weighted EfficientNet ensemble classification with Monte Carlo dropout and TTA, uncertainty quantification, GradCAM++/SHAP explainability, ABCDE morphological analysis, and skin-tone fairness reporting to produce a structured diagnostic output.}
...
The ISIC 2018 data includes 10,015 annotated images in seven categories of diagnoses. The frequencies of classes are defined in Table~\ref{tab:isic2018distribution} with 66.95\% of the images being melanocytic nevus (NV), and 1.15\% being dermatofibroma (DF): the distribution is highly imbalanced.
...
They used a stratified 80/20 partition of ISIC 2018 with a seed of 42, which produces a validation set of 2,003 images as the main benchmark. The resulting data volumes are summarised in Table~\ref{tab:dataset_split}.

\begin{table}[h]
\centering
\caption{Dataset configuration for model training and validation}
\label{tab:dataset_split}
\begin{tabular}{lc}
\hline
Partition & Images \\
\hline
ISIC 2018 Training (80\%) & 8,012 \\
ISIC 2018 Validation (20\%) & 2,003 \\
ISIC 2019 Training (deduplicated) & 23,328 \\
\hline
Combined Training Total & 31,340 \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Classification model training configuration}
...
\hline
Backbone & B3 & B4 & B3 \\
Initialisation & ImageNet & ImageNet & v3.pth \\
Training images & 8,012 & 8,012 & 31,340 \\
Batch size & 32 & 32 & 48 \\
Phase 1 epochs & 8 & 10 & 3 \\
Phase 2 epochs & 22 & 40 & 17 \\
Phase 1 LR & $10^{-3}$ & $10^{-3}$ & $10^{-3}$ \\
Phase 2 LR & $10^{-4}$ & $10^{-4}$ & $10^{-4}$/$5{\times}10^{-4}$ \\
Scheduler & Cosine & Cosine & OneCycleLR \\
Loss & Focal ($\gamma{=}2$) & Focal+Smooth & Focal ($\gamma{=}2$) \\
...
Table~\ref{tab:classification_results} reports overall classification metrics for the proposed ensemble evaluated on the 2,003-image ISIC 2018 validation set.
...
\midrule
MEL  & 0.79 & 0.71 & 0.75 & 223 \\
NV   & 0.93 & 0.97 & 0.95 & 1341 \\
BCC  & 0.85 & 0.91 & 0.88 & 103 \\
AKIEC& 0.82 & 0.69 & 0.75 & 65 \\
BKL  & 0.87 & 0.76 & 0.81 & 220 \\
...
VASC (F1 = 0.98) and NV (F1 -0.95) have the advantage of unique visual patterns and large training support, respectively. The F1 at MEL is 0.75 and AKIEC is 0.75; the major safety issue with MEL is the recall of 0.71, but the uncertainty module marks low-agreement predictions of clinician escalation.
...
\begin{tabular}{lcccc}
\toprule
Model & Data & Accuracy & Macro-F1 & Params \\
\midrule
EfficientNet-B0 (baseline) & 2018 & $\sim$78\% & $\sim$0.65 & 5.3M \\
v3 (EfficientNet-B3) & 2018 & 88.2\% & 0.827 & 10.7M \\
v4 (EfficientNet-B4) & 2018 & 86.3\% & 0.782 & 19.0M \\
\textbf{v5 (EfficientNet-B3)} & \textbf{2018+2019} & \textbf{91.0\%} & \textbf{0.844} & \textbf{10.7M} \\
\bottomrule
\end{tabular}
\end{table}
...
Fig.~\ref{fig:confusion_matrix} shows the confusion matrix of the proposed system on the validation set.
...
\end{axis}
\end{tikzpicture}
\caption{Confusion matrix on the ISIC 2018 validation set (2,003 images). Diagonal entries represent correct classifications. The primary off-diagonal errors occur between MEL and NV and between BKL and NV, consistent with their visual similarity in dermoscopy.}
\label{fig:confusion_matrix}
\end{figure}

\FloatBarrier

The first off-diagnostic error is that MEL is forecasted as NV (21 cases) which is the most clinically significant error. AKIEC-BKL, (8 cases) is an indication of shared surface texture at low magnification of the dermoscopic. The macro-average area under the curve (AUC) of all seven classes is close to 0.97, which supports the fact that the classes are very well separable despite the unequal distribution of labels.
...
(0,0)(0.02,0.71)(0.05,0.82)(0.10,0.89)(0.18,0.93)(0.30,0.97)(1,1)
};
\addlegendentry{Ensemble (AUC $\approx$ 0.97)}
\addplot[dashed,gray] coordinates {(0,0)(1,1)};
\addlegendentry{Chance}
\end{axis}
\end{tikzpicture}
\caption{ROC curve for the weighted ensemble. The curve rises steeply from the origin, reaching a true positive rate of 0.89 at a false positive rate of only 0.10, yielding an estimated AUC of 0.97.}
\label{fig:roc_curve}
\end{figure}

\FloatBarrier

The AUC of 0.97 confirms strong discriminative capability across all operating thresholds. The steep initial rise indicates that the majority of malignant cases are identified at very low false-positive cost, which is the clinically most important region of the curve for a screening application.

\subsection{Segmentation Performance}

Table~\ref{tab:segmentation_results} reports segmentation metrics on a held-out portion of the ISIC 2018 ground truth masks.
...
\end{subfigure}
\caption{System visualisation outputs. The segmentation overlay (left) delineates the lesion boundary; the GradCAM++ heatmap (right) concentrates high-activation regions over the lesion core and irregular border areas, confirming clinically relevant feature attention.}
\label{fig:visual_results}
\end{figure}

The predictions of MEL by GradCAM++ are centered on abnormal pigment networks and blue white veil areas, and even patterns of uniform central globules in NV, which is consistent with current dermoscopic rules, and ensures a model is paying attention to actual lesion morphology instead of the background artefact.
